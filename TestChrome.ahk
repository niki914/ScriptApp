...
; 第4个参数是 msedge.exe 位置，不填会尝试在开始菜单和注册表里找。
...
RunWait, C:\ProgramData\Microsoft\Windows\Start Menu\Programs\Sunshine\Sunshine.lnk
Sleep % sleepTime

SendInput, !{F4}

; ChromeInst := new Chrome("C:\Users\" . A_UserName . "\AppData\Local\Microsoft\Edge\User Data")
ChromeInst := new Chrome("ChromeProfile")

标签 := ChromeInst.GetPage() ; 连接当前标签
标签.Url := "https://localhost:47990/pin"
标签.WaitForLoad() ; 等待网页加载完成

If (Input_State() != 1)
{
  ShowSplashText("Edge" ,"处在中文模式, 已切换", 1000)
  SendInput, {Shift}
}
Sleep % sleepTime
...
标签.getElementById("pin-input").value := "1234"
标签.getElementById("name-input").value := "nm"
标签.WaitForLoad()

标签.querySelector(".btn.btn-primary").click()

ShowSplashText("Edge", "运行完成", sleepTime)
Sleep % sleepTime
SendInput, !{F4}
ExitApp
